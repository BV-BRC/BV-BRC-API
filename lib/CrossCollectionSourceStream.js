...
// Upper bound on target documents one link value may resolve to. Generous
// because the realistic worst case (a highly fragmented draft assembly) is a few
// thousand contigs per genome, and the cost of being wrong is silent data loss.
const DEFAULT_MAX_TARGETS_PER_LINK = 10000
...
class CrossCollectionSourceStream extends Readable {
   ...
   * @param {boolean} [options.emitHeader=true] - Emit a leading Solr-style
   *   metadata document. Solrjs streams do this, and the media serializers
   *   consume it (streamWithBackpressure defaults skipFirstDoc=true), so a
   *   stream that omits it silently loses its first real document.
   */
  constructor (options = {}) {
    ...
    // Max target documents a single link value may resolve to. The link is often
    // one-to-one (sp_gene.feature_id -> one genome_feature), but not always:
    // genome.genome_id -> genome_sequence is one-to-MANY, a genome having as many
    // contigs as its assembly is fragmented. DirectSolrClient.fetchByIds defaults
    // its row cap to values.length, which silently truncates a one-to-many fetch
    // to one document per value — observed as a 105-contig download returning 2
    // records. Cap the fetch generously instead.
    this.maxTargetsPerLink = options.maxTargetsPerLink || DEFAULT_MAX_TARGETS_PER_LINK
    ...
    // Solrjs streams lead with a metadata document, and the media serializers
    // skip it (streamWithBackpressure's skipFirstDoc defaults to true). Match
    // that convention so serializers need no special-casing — without it the
    // first real document is silently dropped from every download.
    this.emitHeader = options.emitHeader !== false
    this.headerEmitted = !this.emitHeader
  ...
  async _fetchTargetDocs (linkValues) {
    ...
    const docs = await this.solrClient.fetchByIds(
      ...
      linkValues,
      {
        ...
        user: this.ctx.user,
        // Explicit cap: the default (values.length) assumes one-to-one.
        rows: linkValues.length * this.maxTargetsPerLink
      }
    )
  ...
  _read () {
    ...
    // Metadata header first, mirroring a Solrjs stream.
    if (!this.headerEmitted) {
      this.headerEmitted = true
      this.push({ response: { numFound: null, start: 0 } })
      return
    }
