...
class DistributedQueryManager {
  ...
  async executeQuery (options) {
    ...
    try {
      ...
      const shardConfigs = shardInfo.map(info => ({
        shard: info.shard,
      ...
      const needsSorted = options.requireSorted || this._hasSortRequirement(options.sort)
   ...
   */
  _hasSortRequirement (sort) {
    if (!sort) return false

    // Check if sort is just the unique key (which is required for cursor pagination anyway)
    // In that case, we don't need merge sort
    const trimmed = sort.trim().toLowerCase()
    if (trimmed === 'id asc' || trimmed === 'id') {
      return false
    }
