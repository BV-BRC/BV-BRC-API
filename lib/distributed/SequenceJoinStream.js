...
class SequenceJoinStream extends Transform {
   ...
   * @param {string} [options.user] - Requesting user. `feature_sequence` IS
   *   publicFree, so permissionFq resolves to null and behavior is unchanged —
   *   threaded for uniformity so a future private sequence collection is safe
   *   by default. See PLAN_ENRICHMENT_PERMISSIONS.md.
  ...
  _startPrefetch (hashes) {
    ...
    return this.solrClient.fetchSequencesByMd5(hashes, {
      permissionFq: this.permissionFq,
      user: this.user
    })
      .catch(err => {
