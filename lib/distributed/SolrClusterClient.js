...
class SolrClusterClient {
  ...
  constructor (solrBaseUrl, options = {}) {
    ...
    // Cache cluster load metrics for 60 seconds by default
    // This is expensive to fetch (requires querying all nodes)
    this.clusterLoadCache = new CacheManager({
      ttlMs: 60 * 1000,
      name: 'clusterLoad'
    })
        ...
        res.on('end', () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            try {
              ...
              reject(new Error(`Failed to parse JSON response from ${sanitizeUrl(url)}: ${err.message}`))
            }
          } else {
            reject(new Error(`HTTP ${res.statusCode} from ${sanitizeUrl(url)}: ${data}`))
          }
        })
      })

      req.on('error', (err) => {
        reject(new Error(`Request failed to ${sanitizeUrl(url)}: ${err.message}`))
      })
  ...
  async getShardsForCollection (collection) {
    ...
    // Resolve Solr aliases (e.g. 'genome' -> 'genome_v02'). CLUSTERSTATUS returns
    // an `aliases` map; a routed alias value may be a comma-separated list of
    // backing collections, in which case we use the first one.
    let resolved = collection
    if (!clusterStatus.collections[resolved] &&
        clusterStatus.aliases && clusterStatus.aliases[collection]) {
      resolved = clusterStatus.aliases[collection].split(',')[0].trim()
      debug(`Resolved alias ${collection} -> ${resolved}`)
    }

    const collectionInfo = clusterStatus.collections[resolved]
    if (!collectionInfo) {
    ...
    for (const [shardName, shardData] of Object.entries(shards)) {
      ...
      // Random selection among all active replicas (both leaders and non-leaders)
      // to spread load across the entire cluster
  ...
  getCacheStats () {
    return {
      ...
      clusterLoad: this.clusterLoadCache.stats()
    }
  }
  ...
  async getNodeMetrics (baseUrl) {
    return new Promise((resolve, reject) => {
      // Inject auth credentials into the URL if we have them
      let metricsUrl = `${baseUrl}/admin/metrics?group=core&group=jvm&wt=json`
      if (this.auth) {
        const parsedBase = new URL(baseUrl)
        parsedBase.username = this.auth.username
        parsedBase.password = this.auth.password
        metricsUrl = `${parsedBase.toString().replace(/\/$/, '')}/admin/metrics?group=core&group=jvm&wt=json`
      }
      ...
      // Use the appropriate agent for connection pooling and SSL settings
      if (this.agent) {
        options.agent = this.agent
      }

      // Handle basic auth from URL (now included via URL parsing)
      if (parsedUrl.username && parsedUrl.password) {
  ...
  _extractLoadMetrics (metrics) {
    ...
    // Solr returns these as direct values: metrics['solr.jvm']['memory.heap.used']
    const jvmMetrics = metrics['solr.jvm'] || {}
    if (typeof jvmMetrics['memory.heap.used'] === 'number') {
      result.heapUsedBytes = jvmMetrics['memory.heap.used']
    }
    if (typeof jvmMetrics['memory.heap.max'] === 'number') {
      result.heapMaxBytes = jvmMetrics['memory.heap.max']
    }
    ...
    // Solr returns: 'QUERY./select.requests': 11903 (direct number)
    // and 'QUERY./select.requestTimes': { count: N, mean_ms: N, ... }
    let totalRequests = 0
    let totalMeanTime = 0
    let coreCount = 0
    ...
    for (const [key, value] of Object.entries(metrics)) {
      if (key.startsWith('solr.core.') && value) {
        ...
        const selectTimes = value['QUERY./select.requestTimes']

        if (typeof selectRequests === 'number') {
          totalRequests += selectRequests
        }
        if (selectTimes && typeof selectTimes.mean_ms === 'number') {
          totalMeanTime += selectTimes.mean_ms
          coreCount++
        }
      }
    }
    ...
    if (coreCount > 0) {
      result.avgQueryTimeMs = Math.round(totalMeanTime / coreCount)
    }
  ...
  async getClusterLoad () {
    return this.clusterLoadCache.getOrFetch('load', async () => {
