...
class GenomeMetadataJoinStream extends Transform {
   ...
   * @param {string} [options.user] - Requesting user. `genome` is NOT publicFree,
   *   so without this the lookup reads other users' private genomes. See
   *   PLAN_ENRICHMENT_PERMISSIONS.md.
  ...
  constructor (solrClient, options = {}) {
    ...
    // Permission scoping for the genome lookup. This stream is constructed
    // per-request, so its cache is not shared across users and needs no scope
    // prefix — but the fetch itself must still be filtered.
    this.permissionFq = buildPermissionFq({
      collection: 'genome',
      user: options.user,
      publicFree: options.publicFree
    })
  ...
  async _fetchAndCacheGenomes (genomeIds) {
    ...
    try {
      const genomeDict = await this.solrClient.fetchGenomeMetadata(
        ...
        { permissionFq: this.permissionFq, user: this.user }
      )
      ...
      // cache with nulls, silently disabling enrichment for those genome ids for
      // the life of the stream. Leave the cache untouched so a later batch
      // retries. See PLAN_ENRICHMENT_PERMISSIONS.md Part 2c.
      debug(`Error fetching genomes (not caching): ${err.message}`)
