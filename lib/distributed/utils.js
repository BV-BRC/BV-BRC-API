 ...
 * @param {Object} [options.agent] - HTTP/HTTPS agent for connection pooling and SSL config
...
async function prewarmShards (shardConfigs, query, options = {}) {
  ...
  for (let i = 0; i < shardConfigs.length; i += maxConcurrent) {
    ...
    const promises = batch.map(config =>
      prewarmSingleShard(config, query, timeout, agent)
        .then(result => {
 ...
 * @param {Object} [agent] - HTTP/HTTPS agent for connection pooling and SSL config
 * @returns {Promise<Object>} { numFound }
 */
function prewarmSingleShard (shardConfig, query, timeout, agent) {
  return new Promise((resolve, reject) => {
    const solrUrl = shardConfig.solrUrl.replace(/\/$/, '')

    // Build prewarm URL with rows=0
    const params = new URLSearchParams()
    params.set('q', '*:*') // Always need a q parameter
    params.set('rows', '0')
    ...
    // Append query filters (fq parameters, etc.)
    ...
    // Use provided agent for connection pooling and SSL configuration
    if (agent) {
