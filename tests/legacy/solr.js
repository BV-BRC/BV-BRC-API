define([
	...
	const basicQueries = [
		["&q=*:*&rows=5", function(data){
			assert.strictEqual(data.response.docs.length, 5);
		}]
	];
...
*/
	// POST Method
	Object.keys(dataModel).filter(m => filter.indexOf(m) == -1).forEach(function(model){
		const Model = dataModel[model];
		const queries = Model.queries ? basicQueries.concat(Model.queries) : basicQueries;
		queries.forEach(function(bq){
			const query = bq[0];
			const handler = bq[1];
