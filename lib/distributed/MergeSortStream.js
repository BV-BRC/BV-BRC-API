...
class MergeSortStream extends Readable {
  ...
  constructor (options) {
    ...
    this.maxHeapDocs = options.maxHeapDocs || config.maxMergeSortHeapDocs
  ...
  _startShards () {
    ...
    for (const shardConfig of this.shardConfigs) {
      ...
      stream.on('data', (doc) => {
        ...
        if (this.heap.size() >= this.maxHeapDocs) {
  ...
  _tryOutput () {
    ...
    while (this._canOutput()) {
      ...
      if (!canContinue) {
        ...
        debug('Output paused due to consumer backpressure')
        return
      }
   ...
   * 2. Every non-exhausted shard has contributed at least one doc to the heap
   *
  ...
  _canOutput () {
    ...
    // We do this by checking if the number of unique shards in heap equals active shards
    // But that's expensive. Instead, track per-shard contribution.

    // Simpler approach: we can output if we have docs from all active shards
    // OR if the heap is very full (beyond a safety threshold)
    // For correctness, we should only output when we have representation from all active shards

    // Count unique shards in heap
    const shardsInHeap = new Set()
    for (const item of this.heap.toArray()) {
      shardsInHeap.add(item.shard)
    }

    // Check each active shard
    for (const [shardName, state] of this.shardStreams) {
      if (!state.exhausted && !shardsInHeap.has(shardName)) {
