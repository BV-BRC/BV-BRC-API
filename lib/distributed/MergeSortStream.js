...
class MergeSortStream extends Readable {
  ...
  _canOutput () {
    ...
    // Count unique shards currently in the heap
    ...
    if (missingShards.length > 0) {
      // Debug: log missing shards occasionally
      if (this.stats.heapPushes % 5000 === 0 && this.stats.heapPushes > 0) {
        debug(`_canOutput: waiting for ${missingShards.length} shards: ${missingShards.slice(0, 5).join(', ')}${missingShards.length > 5 ? '...' : ''}`)
      }
      return false
    }
