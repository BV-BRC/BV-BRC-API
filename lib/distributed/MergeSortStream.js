...
class MergeSortStream extends Readable {
  ...
  constructor (options) {
    ...
    // Rate-limit heap full / shard resume logging
    this.lastHeapFullLog = 0
    this.heapFullCount = 0
    this.lastShardResumeLog = 0
    this.shardResumeCount = 0
    ...
    for (const shardConfig of this.shardConfigs) {
      ...
      stream.on('data', (doc) => {
        ...
        if (this.heap.size() >= this.maxHeapDocs && shardState.hasContributed) {
          // Rate-limit heap full logging
          this.heapFullCount++
          const now = Date.now()
          if (now - this.lastHeapFullLog > 5000) {
            debug(`Heap full (${this.heap.size()} docs), paused ${this.heapFullCount} shards total`)
            this.lastHeapFullLog = now
          }
  ...
  _canOutput () {
    ...
    for (const [shardName, state] of this.shardStreams) {
      if (!state.exhausted && !this.shardDocsInHeap.has(shardName)) {
        missingShards.push(shardName)
      }
    }

    if (missingShards.length > 0) {
      // Debug: log which shards are missing (rate limited)
      if (this.stats.heapPushes % 10000 === 0) {
        debug(`_canOutput: waiting for ${missingShards.length} shards to have docs in heap: ${missingShards.slice(0, 5).join(', ')}${missingShards.length > 5 ? '...' : ''}`)
      }
      return false
    }
  ...
  _resumePausedShards () {
    ...
    // Rate-limit resume logging
    if (resumedCount > 0) {
      this.shardResumeCount += resumedCount
      const now = Date.now()
      if (now - this.lastShardResumeLog > 5000) {
        debug(`Resumed ${this.shardResumeCount} shards total (heap at ${this.heap.size()} docs)`)
        this.lastShardResumeLog = now
        this.shardResumeCount = 0
      }
    }
  }
  ...
  _read () {
    ...
    // Always try to output - either we're resuming from backpressure,
    // or we might now be able to output after more shards contributed
    this.outputPaused = false
    this._tryOutput()
  }
