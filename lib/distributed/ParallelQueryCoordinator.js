...
class ParallelQueryCoordinator extends Readable {
  ...
  constructor (options) {
    ...
    this.drainBatchSize = 100 // Drain when buffer reaches this size
    // Completion state. EOF (push(null)) must not be emitted until every buffered
    // document has been pushed downstream; otherwise a resume after EOF calls
    // push() after EOF (ERR_STREAM_PUSH_AFTER_EOF) and trailing docs are lost.
    this.allShardsComplete = false
    this.ended = false
   ...
   */
  _drainBuffer () {
    if (this.paused || this.destroyed || this.ended) {
      return
    }
    ...
    while (this.documentBuffer.length > 0) {
      ...
      if (!canContinue) {
        ...
        this.paused = true
        // Only log significant backpressure events
        if (pushed > 100 || this.documentBuffer.length > 1000) {
    ...
    // Only log significant drains
    if (pushed >= 1000) {
      debug(`Drained ${pushed} docs from buffer`)
    }

    // Buffer is fully drained without backpressure. If every shard has already
    // completed, this is the moment to emit EOF — never before the buffer empties.
    this._maybeEnd()
  }

  /**
   * Emit EOF once all shards are complete AND the buffer is fully drained.
   * Idempotent: guarded by this.ended so push(null) happens exactly once.
   */
  _maybeEnd () {
    if (this.ended || this.destroyed) return
    if (this.allShardsComplete && this.documentBuffer.length === 0) {
      this.ended = true
      debug('All shards complete and buffer drained, ending stream')
      this.push(null)
    }
  }
  ...
  _checkCompletion () {
    ...
    if (allComplete) {
      ...
      // Mark completion, then drain. _drainBuffer -> _maybeEnd emits EOF only
      // once the buffer is empty. If the consumer is currently backpressured, the
      // drain stops short and EOF is deferred until a later resume/_read drains
      // the remainder — pushing null now would drop those trailing docs and, on
      // the next resume, crash with ERR_STREAM_PUSH_AFTER_EOF.
      this.allShardsComplete = true
      this._drainBuffer()
    }
  }
  ...
  _read () {
    ...
    if (!this.started) {
      debug(`_read: Starting initial shards`)
